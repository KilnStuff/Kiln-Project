...
There were several considerations that needed to be taken into account when constructing this project. The first main challenge was how to install the thermocouples. Previously they were placed inside of the portholes that already existed on the kiln, and were removed temporarily whenever we needed to look inside of the kiln. However, with the PID controlling the kiln, removing the thermocouples was no longer an option as this sudden change in measured temperature would throw off the controller, driving the power way up inside the kiln (when in fact is does not need to). To allow for port holes to still be accessible so that we could look inside the kiln, we needed to drill additional holes for the thermocouples. However while drilling these new holes we had to take into account that we were drilling fire brick, so we needed to take special consideration. Fire brick is especially soft and brittle, and we needed to be very gentle while drilling the holes in order to avoid breaking the bricks. To make things more difficult the kiln is also wrapped in a steel sheet metal, which is not easy to drill through gently. We decided that the best way to go about drilling these holes would be to first drill the sheet metal by starting with a small bit and slowly incrementing up in bit size to the final hole size. By slowly going up in bit size we minimized the shock we put into the firebrick each time we punched through the fire brick. Then we drilled from the inside of the kiln out through the fire brick. We decided to drill out this way because the sheet metal will support the backside of the brick and help prevent any blowout while drilling. When drilling the firebrick it would probably be best to use a masonry bit, but we had no issues using a standard drill bit due to how soft the material is. Also be sure to **not** use a hammer drill or you will destroy the firebrick.
...
The first constants we acquired were the coefficients for power loss out of each of the zones. We measured these by going through each zone and setting one of the zones to full power then measuring the temperatures at equilibrium. By assuming the power loss scales linearly with the difference with respect to external temperature this gave us a system of three equations with three unknowns allowing us to solve for dissipation coefficients, which had a value of about 2 to 0.5. The next challenge was to find the coefficients of the PID itself. We decided to set the value of P for each zone by choosing a minimum value for the error at full power. We decide this value to be 10 degrees, meaning that if the kiln was more than 10 degrees below the set point it would apply full power to the coils. P = (maxPower)/(minError) ~ 100 to 200. We decided that we wanted the values of I to be on the same order of magnitude as the dissipative constants because the have a similar effect while still being proportional to the power that the coils can deliver. So we just scaled down our value of P down by 100 to get I. For the D coefficient we wanted it to still be proportional to the max power of the coils, and because sticking to a specific ramp rate is crucial to our process, we decided keep it relatively large compared to I. This lead us to decide D = P/10. However during later tests we noticed some oscillations that we thought were from the derivative term so we decreased this to P/20.
